// ============================================
// PREPARAR ESTADÍSTICAS DE CHAT - VERSIÓN TESTING
// ============================================

// ⚠️ MODO TESTING - Comentá/descomentá estas secciones según necesites

// ========== MODO TESTING ==========
// Descomentar para usar datos simulados
const TESTING_MODE = true;

let formatData, webhookData, body, tokensHaiku, tokensSonnet, propIds;

if (TESTING_MODE) {
  // ✅ DATOS SIMULADOS PARA TESTING
  formatData = $input.first().json;

  webhookData = {
    body: {
      sessionId: 'test-session-' + Date.now(),
      message: 'tenes deptos para alquilar en palermo',
  ...
  tokensHaiku = 301;
  tokensSonnet = 2536;
  propIds = ['PROP-001', 'PROP-004'];

} else {
  // ❌ MODO PRODUCCIÓN - Obtener datos reales de nodos anteriores
  formatData = $input.first().json;
  webhookData = $('Webhook Chat').first().json;
  ...
  if (!formatData.error) {
    try {
      ...
      // Sumar input_tokens + output_tokens (Anthropic no da total_tokens)
      if (haikuData?.usage) {
      ...
      propIds = sonnetPrep?.idsSeleccionados || [];
...
// Preparar datos para PostgreSQL con query SQL directo
...
const propiedadesMostradasInt = parseInt(propIds.length, 10);
const propiedadesIdsArray = propIds.length > 0 ? propIds : [];
...
if (propiedadesIdsArray.length > 0) {
  const escapedProps = propiedadesIdsArray.map(p => escapeSql(p)).join(',');
  propiedadesIdsSQL = 'ARRAY[' + escapedProps + ']';
}

// Construir query SQL completo (sin parámetros preparados, valores directos)
...
return [{
  json: {
    query: query,
    // Pasar también la respuesta original para el webhook
    _responseOriginal: formatData
  }
}];
