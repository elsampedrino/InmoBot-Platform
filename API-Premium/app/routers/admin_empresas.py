...
def _to_response(e) -> EmpresaAdminResponse:
    ...
    return EmpresaAdminResponse(
        ...
        servicios=EmpresaServiciosSchema(
            ...
            facebook=srv.get("facebook", False),
            canal_web=srv.get("canal_web", True),
            canal_whatsapp=srv.get("canal_whatsapp", False),
        ),
...
async def create_empresa(
    ...
    empresa = await repo.create_empresa(db, body.model_dump(exclude={"id_rubro"}))
    await db.flush()
    db.add(EmpresaRubro(
